(() => {
  ...
  const state = {
    ...
    gristOptionsLoaded: false, gristAccessLevel: "", noteStorageState: "loading", noteStorageError: "", activeRecord: null
  };
  const dom = {};
  ...
  function cacheDom() {
    [
      ...
      "noteSaveBtn", "noteDeleteBtn", "noteCancelBtn", "noteCloseBtn", "recordModal", "recordModalTitle", "recordModalSubtitle",
      "recordCloseBtn", "recordCloseActionBtn", "recordShareJpegBtn", "recordCopyRefBtn", "recordHeroDeveloper", "recordHeroReference",
      "recordHeroStatus", "recordDetailsGrid", "recordGeneratedAt", "pageSizeSelect", "pdfScopeSelect", "pdfExportBtn", "tableBody", "pageInfo", "prevPageBtn",
  ...
  function bindEvents() {
    ...
    dom.pdfExportBtn.addEventListener("click", exportTablePdf);
    ...
    dom.recordCloseBtn.addEventListener("click", closeRecordModal);
    dom.recordCloseActionBtn.addEventListener("click", closeRecordModal);
    dom.recordShareJpegBtn.addEventListener("click", shareActiveRecordAsJpeg);
    dom.recordCopyRefBtn.addEventListener("click", copyActiveRecordReference);
    dom.recordModal.querySelectorAll("[data-record-close]").forEach((el) => el.addEventListener("click", closeRecordModal));
    document.addEventListener("keydown", (event) => {
      if (event.key === "Escape" && !dom.recordModal.hidden) closeRecordModal();
      else if (event.key === "Escape" && !dom.noteModal.hidden) closeNoteModal();
    });
  }
    ...
    if (!pageRows.length) {
      ...
      dom.tableBody.innerHTML = pageRows.map((r, index) => {
        ...
        return `<tr class="${selected}" data-index="${index}" tabindex="0">
          ...
          <td><button class="application-detail-link" type="button" data-record-detail="${index}" title="Buka butiran lengkap permohonan"><span class="cell-clamp">${escapeHtml(r.Permohonan || "–")}</span><span class="application-link-icon" aria-hidden="true">↗</span></button></td>
          <td><strong>${escapeHtml(r.NoRujPKM || "–")}</strong></td>
          <td>${escapeHtml(r.Lot || "–")}</td>
          <td>${escapeHtml(r.MukimResolved)}</td>
          <td>${formatDate(r.OscTerimaDate)}</td>
          <td><div class="cell-clamp">${escapeHtml(r.TunaiSyarat || "–")}</div></td>
          <td>${formatDate(r.BentangOSCDate) || escapeHtml(textValue(r.BentangOSC) || "–")}</td>
          <td>${escapeHtml(r.BilOSC || "–")}</td>
          <td>${formatDate(r.TarikhOSCDate)}</td>
          ...
          <td>${formatDate(r.TarikhC1Date)}</td>
          ...
          <td class="numeric">${escapeHtml(textValue(r.Latitude) || "–")}</td>
          <td class="numeric">${escapeHtml(textValue(r.Longitude) || "–")}</td>
        </tr>`;
      }).join("");
      ...
      dom.tableBody.querySelectorAll("button[data-record-detail]").forEach((button) => {
        button.addEventListener("keydown", (event) => event.stopPropagation());
        button.addEventListener("click", (event) => {
          event.stopPropagation();
          const record = pageRows[Number(button.dataset.recordDetail)];
  ...
  const RECORD_DETAIL_SECTIONS = [
    { title: "Maklumat Permohonan", fields: [
      ["Pemaju", (r) => r.Pemaju],
      ["Permohonan", (r) => r.Permohonan, "wide"],
      ["No. Ruj. PKM", (r) => r.NoRujPKM]
    ]},
    { title: "Maklumat Tanah", fields: [
      ["No. Lot", (r) => r.Lot],
      ["Mukim", (r) => r.MukimResolved]
    ]},
    { title: "Proses OSC", fields: [
      ["OSC Terima", (r) => formatDate(r.OscTerimaDate)],
      ["Tunai Syarat", (r) => r.TunaiSyarat],
      ["Bentang OSC", (r) => formatDate(r.BentangOSCDate) || textValue(r.BentangOSC)],
      ["Bil. OSC", (r) => r.BilOSC],
      ["Tarikh OSC", (r) => formatDate(r.TarikhOSCDate)]
    ]},
    { title: "Keputusan dan Pematuhan", fields: [
      ["Keputusan", (r) => r.KeputusanResolved, "decision"],
      ["Status Tunai Syarat", (r) => r.StatusTunaiSyarat || "Tiada rekod", "wide"],
      ["Tarikh C1", (r) => formatDate(r.TarikhC1Date)]
    ]},
    { title: "Maklumat Sokongan", fields: [
      ["Perunding", (r) => r.Perunding],
      ["Pindaan", (r) => formatBooleanLike(r.Pindaan)],
      ["Latitude", (r) => textValue(r.Latitude)],
      ["Longitude", (r) => textValue(r.Longitude)]
    ]}
  ];
  ...
  function openRecordModal(record) {
    ...
    dom.recordModalSubtitle.textContent = record.NoRujPKM ? `No. Ruj. PKM: ${record.NoRujPKM}` : "No. Ruj. PKM belum direkodkan";
    dom.recordHeroDeveloper.textContent = record.Pemaju || "Pemaju tidak dinyatakan";
    dom.recordHeroReference.textContent = record.NoRujPKM || "Tiada No. Ruj. PKM";
    dom.recordHeroStatus.className = `status-badge status-${record.DecisionClass}`;
    dom.recordHeroStatus.textContent = record.KeputusanResolved;
    dom.recordGeneratedAt.textContent = `Dibuka ${new Intl.DateTimeFormat("ms-MY", { day: "2-digit", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" }).format(new Date())}`;
    dom.recordDetailsGrid.innerHTML = RECORD_DETAIL_SECTIONS.map((section) => `
      <section class="record-detail-section">
        <h4>${escapeHtml(section.title)}</h4>
        <div class="record-field-grid">
          ${section.fields.map(([label, getter, type]) => {
            const value = getter(record) || "–";
            const extraClass = type === "wide" ? " record-field-wide" : type === "decision" ? " record-field-decision" : "";
            const display = type === "decision" ? `<span class="status-badge status-${record.DecisionClass}">${escapeHtml(value)}</span>` : `<p>${escapeHtml(value)}</p>`;
            return `<div class="record-field${extraClass}"><span>${escapeHtml(label)}</span>${display}</div>`;
          }).join("")}
        </div>
      </section>`).join("");
    dom.recordModal.hidden = false;
    document.body.classList.add("modal-open");
    window.setTimeout(() => dom.recordShareJpegBtn.focus(), 0);
  }
  ...
  async function copyActiveRecordReference() {
    const value = state.activeRecord?.NoRujPKM;
    if (!value) return showToast("No. Ruj. PKM belum direkodkan.");
    try {
      await navigator.clipboard.writeText(value);
      showToast("No. Ruj. PKM telah disalin.");
    } catch (_) {
  ...
  async function shareActiveRecordAsJpeg() {
    const record = state.activeRecord;
    if (!record) return;
    const originalLabel = dom.recordShareJpegBtn.textContent;
    dom.recordShareJpegBtn.disabled = true;
    dom.recordShareJpegBtn.textContent = "Menjana JPEG...";
    try {
      const blob = await createRecordJpeg(record);
      const filename = `PKM_${safeFilename(record.NoRujPKM || record.Pemaju || "REKOD")}_${compactDate(new Date())}.jpg`;
      const file = new File([blob], filename, { type: "image/jpeg" });
      if (navigator.share && navigator.canShare?.({ files: [file] })) {
        try {
          await navigator.share({
            title: "Butiran Permohonan Kebenaran Merancang",
            text: `${record.NoRujPKM || "Rekod PKM"} – ${record.Pemaju || ""}`.trim(),
            files: [file]
          });
          showToast("Menu perkongsian JPEG telah dibuka.");
          return;
        } catch (error) {
      ...
      showToast("JPEG telah dimuat turun. Fail boleh dikongsi melalui WhatsApp atau aplikasi lain.");
    } catch (error) {
      console.error(error);
      showToast("JPEG tidak dapat dijana. Sila cuba semula.");
    } finally {
      dom.recordShareJpegBtn.disabled = false;
      dom.recordShareJpegBtn.textContent = originalLabel;
    }
  }

  function createRecordJpeg(record) {
    return new Promise((resolve, reject) => {
      try {
        ...
        const fields = [
          ["PEMAJU", record.Pemaju || "–"],
          ["PERMOHONAN", record.Permohonan || "–"],
          ["NO. RUJ. PKM", record.NoRujPKM || "–"],
          ["NO. LOT", record.Lot || "–"], ["MUKIM", record.MukimResolved || "–"],
          ["OSC TERIMA", formatDate(record.OscTerimaDate)], ["TUNAI SYARAT", record.TunaiSyarat || "–"],
          ["BENTANG OSC", formatDate(record.BentangOSCDate) || textValue(record.BentangOSC) || "–"],
          ["BIL. OSC", record.BilOSC || "–"], ["TARIKH OSC", formatDate(record.TarikhOSCDate)],
          ["KEPUTUSAN", record.KeputusanResolved], ["STATUS TUNAI SYARAT", record.StatusTunaiSyarat || "Tiada rekod"],
          ["TARIKH C1", formatDate(record.TarikhC1Date)], ["PERUNDING", record.Perunding || "–"],
          ["PINDAAN", formatBooleanLike(record.Pindaan)], ["LATITUDE", textValue(record.Latitude) || "–"],
          ["LONGITUDE", textValue(record.Longitude) || "–"]
        ];
        ctx.font = `600 27px ${font}`;
        const colGap = 28;
        const colWidth = (contentWidth - colGap) / 2;
        const measured = fields.map(([label, value], idx) => {
          const span = idx === 1 ? 2 : 1;
          const boxWidth = span === 2 ? contentWidth : colWidth;
          ctx.font = `600 27px ${font}`;
          const lines = wrapCanvasText(ctx, String(value || "–"), boxWidth - 48);
          return { label, value: String(value || "–"), span, lines, height: 54 + Math.max(1, lines.length) * 38 + 26 };
        });
        let bodyHeight = 0;
        let rowHeight = 0;
        let occupied = 0;
        measured.forEach((item) => {
          if (item.span === 2) {
            if (occupied) { bodyHeight += rowHeight + 22; occupied = 0; rowHeight = 0; }
            bodyHeight += item.height + 22;
          } else {
            rowHeight = Math.max(rowHeight, item.height); occupied += 1;
            if (occupied === 2) { bodyHeight += rowHeight + 22; occupied = 0; rowHeight = 0; }
          }
        });
        if (occupied) bodyHeight += rowHeight + 22;
        canvas.width = width;
        canvas.height = 390 + bodyHeight + 150;
        ctx.fillStyle = "#F3F4F7";
        ctx.fillRect(0, 0, canvas.width, canvas.height);
        ctx.fillStyle = "#2C3240";
        roundedRect(ctx, 52, 48, width - 104, 250, 34);
        ctx.fill();
        ctx.fillStyle = "#AEB6C8";
        ctx.font = `700 22px ${font}`;
        ctx.fillText("DASHBOARD PERMOHONAN KEBENARAN MERANCANG", pad, 104);
        ctx.fillStyle = "#FFFFFF";
        ctx.font = `800 48px ${font}`;
        ctx.fillText("DAERAH SEBERANG PERAI SELATAN", pad, 166);
        ctx.font = `600 25px ${font}`;
        ctx.fillStyle = "#D7DBE3";
        ctx.fillText("MAJLIS BANDARAYA SEBERANG PERAI", pad, 208);
        const statusColor = DECISION_COLORS[record.DecisionClass] || DECISION_COLORS.other;
        ctx.font = `700 23px ${font}`;
        const statusText = record.KeputusanResolved;
        const statusW = Math.min(400, ctx.measureText(statusText).width + 52);
        ctx.fillStyle = statusColor;
        roundedRect(ctx, width - pad - statusW, 226, statusW, 52, 26);
        ctx.fill();
        ctx.fillStyle = "#FFFFFF";
        ctx.textAlign = "center";
        ctx.fillText(statusText, width - pad - statusW / 2, 260);
        ctx.textAlign = "left";
        let x = pad, y = 340, currentRowHeight = 0, col = 0;
        measured.forEach((item) => {
          if (item.span === 2) {
            if (col) { y += currentRowHeight + 22; x = pad; col = 0; currentRowHeight = 0; }
            drawJpegField(ctx, item, x, y, contentWidth, item.height, font);
            y += item.height + 22;
          } else {
            drawJpegField(ctx, item, x, y, colWidth, item.height, font);
            currentRowHeight = Math.max(currentRowHeight, item.height);
            col += 1;
            if (col === 2) { y += currentRowHeight + 22; x = pad; col = 0; currentRowHeight = 0; }
            else x = pad + colWidth + colGap;
          }
        });
        if (col) y += currentRowHeight + 22;
        ctx.strokeStyle = "#D7DBE3";
        ctx.lineWidth = 2;
        ctx.beginPath(); ctx.moveTo(pad, canvas.height - 92); ctx.lineTo(width - pad, canvas.height - 92); ctx.stroke();
        ctx.fillStyle = "#6E7A94";
        ctx.font = `600 20px ${font}`;
        ctx.fillText(`Dijana pada ${new Intl.DateTimeFormat("ms-MY", { dateStyle: "medium", timeStyle: "short" }).format(new Date())}`, pad, canvas.height - 48);
        ctx.textAlign = "right";
        ctx.fillText(record.NoRujPKM || "REKOD PKM", width - pad, canvas.height - 48);
        ctx.textAlign = "left";
        canvas.toBlob((blob) => blob ? resolve(blob) : reject(new Error("Canvas kosong")), "image/jpeg", .94);
      } catch (error) { reject(error); }
    });
  }

  function drawJpegField(ctx, item, x, y, width, height, font) {
    ctx.fillStyle = "#FFFFFF";
    ctx.strokeStyle = "#D7DBE3";
    ctx.lineWidth = 2;
    ...
    ctx.fillText(item.label, x + 24, y + 34);
    ctx.fillStyle = "#2C3240";
    ctx.font = `600 27px ${font}`;
    item.lines.forEach((line, index) => ctx.fillText(line, x + 24, y + 76 + index * 38));
  }
  ...
  function safeFilename(value) {
    return String(value || "REKOD").normalize("NFKD").replace(/[^a-zA-Z0-9_-]+/g, "_").replace(/^_+|_+$/g, "").slice(0, 80) || "REKOD";
  }
  ...
  const TABLE_EXPORT_HEADERS = [
    "PEMAJU", "PERMOHONAN", "NO. RUJ. PKM", "NO. LOT", "MUKIM", "OSC TERIMA", "TUNAI SYARAT",
    "BENTANG OSC", "BIL. OSC", "TARIKH OSC", "KEPUTUSAN", "STATUS TUNAI SYARAT", "TARIKH C1",
    "PERUNDING", "PINDAAN", "LATITUDE", "LONGITUDE"
  ];

  function tableExportRows(records) {
    return records.map((r) => [
      r.Pemaju, r.Permohonan, r.NoRujPKM, r.Lot, r.MukimResolved,
      isoDate(r.OscTerimaDate), r.TunaiSyarat,
      isoDate(r.BentangOSCDate) || textValue(r.BentangOSC), r.BilOSC,
      isoDate(r.TarikhOSCDate), r.KeputusanResolved, r.StatusTunaiSyarat,
      isoDate(r.TarikhC1Date), r.Perunding, formatBooleanLike(r.Pindaan),
      textValue(r.Latitude), textValue(r.Longitude)
    ]);
  }

  function recordsForPdfScope() {
    const sorted = sortRecords(state.filtered);
    if (dom.pdfScopeSelect?.value !== "page") return sorted;
    const start = (state.page - 1) * state.pageSize;
    return sorted.slice(start, start + state.pageSize);
  }

  function exportFilteredCsv() {
    const rows = tableExportRows(sortRecords(state.filtered));
    const csv = "\ufeff" + [TABLE_EXPORT_HEADERS, ...rows].map((row) => row.map(csvCell).join(",")).join("\r\n");
    ...
    showToast(`${formatNumber(rows.length)} rekod telah dieksport dalam susunan jadual semasa.`);
  }

  function exportTablePdf() {
    const records = recordsForPdfScope();
    if (!records.length) {
      showToast("Tiada rekod untuk dimuat turun sebagai PDF.");
      return;
    }
    const JsPdf = window.jspdf?.jsPDF;
    if (!JsPdf) {
      showToast("Modul PDF belum dimuatkan. Sila muat semula dashboard.");
      return;
    }
    const doc = new JsPdf({ orientation: "landscape", unit: "mm", format: "a3", compress: true });
    if (typeof doc.autoTable !== "function") {
      showToast("Modul jadual PDF belum tersedia. Sila muat semula dashboard.");
      return;
    }

    const rows = tableExportRows(records).map((row) => row.map(pdfSafeText));
    const scopeLabel = dom.pdfScopeSelect?.value === "page" ? `Halaman semasa (${state.page})` : "Semua rekod ditapis";
    const filterLabel = pdfFilterSummary();
    const generatedAt = new Intl.DateTimeFormat("ms-MY", {
      day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit"
    }).format(new Date());

    doc.setProperties({
      title: "Jadual Analisis Permohonan Kebenaran Merancang SPS",
      subject: scopeLabel,
      author: "Majlis Bandaraya Seberang Perai",
      creator: "Dashboard PKM SPS"
    });
    doc.setTextColor(44, 50, 64);
    doc.setFont("helvetica", "bold");
    doc.setFontSize(12);
    doc.text("JADUAL ANALISIS PERMOHONAN KEBENARAN MERANCANG", 8, 10);
    doc.setFontSize(10);
    doc.text("DAERAH SEBERANG PERAI SELATAN", 8, 15);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(7);
    doc.text(`MAJLIS BANDARAYA SEBERANG PERAI  |  ${scopeLabel}  |  ${formatNumber(records.length)} rekod`, 8, 20);
    const filterLines = doc.splitTextToSize(`Penapis: ${filterLabel}`, 300);
    doc.text(filterLines, 8, 24);
    doc.text(`Dijana: ${generatedAt}`, 412, 20, { align: "right" });

    const decisionColumnIndex = 10;
    doc.autoTable({
      head: [TABLE_EXPORT_HEADERS.map(pdfSafeText)],
      body: rows,
      startY: 28 + Math.max(0, filterLines.length - 1) * 3,
      margin: { top: 8, right: 8, bottom: 10, left: 8 },
      theme: "grid",
      showHead: "everyPage",
      styles: {
        font: "helvetica", fontSize: 5.2, cellPadding: 1.05, overflow: "linebreak",
        valign: "middle", textColor: [44, 50, 64], lineColor: [215, 219, 227], lineWidth: 0.12,
        minCellHeight: 4.2, minCellWidth: 0
      },
      headStyles: {
        fillColor: [44, 50, 64], textColor: [255, 255, 255], fontStyle: "bold",
        fontSize: 5.4, halign: "center", valign: "middle", lineColor: [44, 50, 64]
      },
      alternateRowStyles: { fillColor: [243, 244, 247] },
      columnStyles: {
        0: { cellWidth: 27 }, 1: { cellWidth: 52 }, 2: { cellWidth: 27 }, 3: { cellWidth: 17 },
        4: { cellWidth: 13 }, 5: { cellWidth: 18 }, 6: { cellWidth: 23 }, 7: { cellWidth: 18 },
        8: { cellWidth: 11 }, 9: { cellWidth: 18 }, 10: { cellWidth: 18 }, 11: { cellWidth: 27 },
        12: { cellWidth: 18 }, 13: { cellWidth: 28 }, 14: { cellWidth: 13 }, 15: { cellWidth: 18 },
        16: { cellWidth: 18 }
      },
      didParseCell(data) {
        if (data.section !== "body") return;
        if (data.column.index === decisionColumnIndex) {
          const decisionClass = records[data.row.index]?.DecisionClass || "other";
          const rgb = hexToRgb(DECISION_COLORS[decisionClass] || DECISION_COLORS.other);
    ...
    for (let page = 1; page <= totalPages; page += 1) {
      doc.setPage(page);
      doc.setDrawColor(215, 219, 227);
      doc.line(8, pageHeight - 7, pageWidth - 8, pageHeight - 7);
      doc.setFont("helvetica", "normal");
      doc.setFontSize(6.5);
      doc.setTextColor(110, 122, 148);
      doc.text("Dashboard PKM SPS - Dokumen analisis dalaman", 8, pageHeight - 3.5);
      doc.text(`Halaman ${page} daripada ${totalPages}`, pageWidth - 8, pageHeight - 3.5, { align: "right" });
    }

    const scopeCode = dom.pdfScopeSelect?.value === "page" ? `HALAMAN_${state.page}` : "DITAPIS";
    doc.save(`PKM_SPS_JADUAL_${scopeCode}_${compactDate(new Date())}.pdf`);
    showToast(`${formatNumber(records.length)} rekod telah dimuat turun sebagai PDF.`);
  }
  ...
  function pdfFilterSummary() {
    ...
    const add = (label, element) => {
      ...
      if (!value) return;
      const text = element.tagName === "SELECT" ? element.options[element.selectedIndex]?.text : value;
      items.push(`${label}: ${text}`);
    };
    add("Carian", dom.searchInput);
    add("Tahun", dom.yearFilter);
    add("Bulan", dom.monthFilter);
    add("Mukim", dom.mukimFilter);
    add("Keputusan", dom.decisionFilter);
    add("Pemaju", dom.developerFilter);
    add("Perunding", dom.consultantFilter);
    add("Status Tunai Syarat", dom.complianceFilter);
    return items.length ? items.map(pdfSafeText).join(" | ") : "Tiada penapis aktif";
  }
  ...
  function hexToRgb(hex) {
    const clean = String(hex || "#2C3240").replace("#", "");
